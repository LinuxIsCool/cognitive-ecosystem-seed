...
# Configuration
set CURRENT_DIR (pwd)
set TEMPLATES_PATH "$CURRENT_DIR/.templates"
set CURRENT_DATE (date +%Y-%m-%d)
set DEFAULT_EDITOR "nvim" # Change to your preferred editor
...
if test -d "$CURRENT_DIR/journal" -o -d "$CURRENT_DIR/.templates"
    log_warn "Some system directories already exist in: $CURRENT_DIR"
    read -l -P "Continue and potentially overwrite existing files? (y/N) " confirm
...
set SEED_TEMPLATE "$TEMPLATES_PATH/01-seed-node.md"
echo "---" > $SEED_TEMPLATE
...
set DAILY_TEMPLATE "$TEMPLATES_PATH/10-daily-note.md"
echo "---" > $DAILY_TEMPLATE
...
set PROJECT_TEMPLATE "$TEMPLATES_PATH/20-project-note.md"
...
set KNOWLEDGE_TEMPLATE "$TEMPLATES_PATH/30-knowledge-note.md"
...
set ENV_TEMPLATE "$TEMPLATES_PATH/50-env.template"
...
set SEED_NODE "$CURRENT_DIR/people/01-seed-$USERNAME.md"
...
set TODAY_NOTE "$CURRENT_DIR/journal/daily/$CURRENT_DATE.md"
cat "$TEMPLATES_PATH/10-daily-note.md" | sed "s/{{date}}/$CURRENT_DATE/g" | sed "s/{{date_pretty}}/"(date '+%A, %B %-d, %Y')"/g" > $TODAY_NOTE
log_done "Today's note created at $TODAY_NOTE"

# Create index files
log_step "Creating index files..."
# Journal index
set JOURNAL_INDEX "$CURRENT_DIR/journal/00-index.md"
...
# Set up fish abbreviation directly in current shell and in config
log_step "Setting up fish abbreviation..."
set REPO_PATH (realpath $CURRENT_DIR)

# Create the abbreviation in the current shell
abbr -a s "cd \"$REPO_PATH\" && $DEFAULT_EDITOR \"people/01-seed-$USERNAME.md\""

# Also add it to the config file if not already there
if not contains "abbr -Ua s" ~/.config/fish/config.fish
    echo "" >> ~/.config/fish/config.fish
    echo "# Cognitive Ecosystem - quick access to seed node" >> ~/.config/fish/config.fish
    echo "abbr -Ua s 'cd \"$REPO_PATH\" && $DEFAULT_EDITOR \"people/01-seed-$USERNAME.md\"'" >> ~/.config/fish/config.fish
    log_done "Added abbreviation 's' to fish config and current shell"
else
    log_warn "Abbreviation 's' already exists in fish config"
    log_done "Added abbreviation 's' to current shell only"
end

# Create next steps document
log_step "Creating next steps document..."
set NEXT_STEPS "$CURRENT_DIR/00-NEXT-STEPS.md"
